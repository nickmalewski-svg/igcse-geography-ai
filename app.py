# Required libraries
# pip install openai streamlit pandas regex pdfkit supabase requests

import openai, streamlit as st, pandas as pd, re, pdfkit, requests
from supabase import create_client
from datetime import datetime

# -----------------------------
# 1️⃣ Supabase & OpenAI Setup
# -----------------------------
...
if page == "🏠 Home":
    ...
    - Take **adaptive mock exams** tailored to your weak topics.
    - Answer **MCQs, short answers, essays, and map-based questions**.
    - Receive **AI-generated feedback and PDF reports**.
    - Track your progress with a **dashboard and badges**.
    """)
...
if page == "📝 Take Exam":
    ...
    def calculate_mastery(past_results, topics):
        mastery = {}
        for t in topics:
            topic_scores = past_results[past_results["topics"].str.contains(t)]
    ...
    if "exam_text" in st.session_state:
        ...
        if st.button("Submit & Get Feedback"):
            with st.spinner("Grading exam..."):
                ...
                html_content=f"<h1>Exam Report for {user_email}</h1><pre>{st.session_state['exam_text']}\n\n{feedback_text}</pre>"
